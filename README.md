...
Solução: Na exclusão do registro o delete já pega o próximo registro, com o next ele pulava um novo registro e com isto não verifica todos os registros. Foi colocado um if para buscar o próximo registro apenas quando for impar.
...
Solução: Habilitada diretiva IMAGE_FILE_LARGE_ADDRESS_AWARE e realizando limpeza no DataSet quando o arquivo atingir um limite (100MB).
...
Solução: Ao enviar os dados para o servidor a pasta é de arquivos do servidor limpa e na exceção também deve ser limpa a pasta.